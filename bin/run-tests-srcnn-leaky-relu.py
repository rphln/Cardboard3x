...
from torch.nn import Conv2d, Module, init
from torch.nn.functional import interpolate, l1_loss, mse_loss, relu, leaky_relu
from torchvision.transforms.functional import normalize
...
class SRCNN(Module):
    ...
    def forward(self, x):
        ...
        x = leaky_relu(self.conv1(x), negative_slope=0.01, inplace=True)
        x = leaky_relu(self.conv2(x), negative_slope=0.01, inplace=True)
        x = self.conv3(x)
